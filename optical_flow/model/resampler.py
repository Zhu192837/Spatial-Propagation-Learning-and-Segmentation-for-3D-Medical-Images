...
def resampler_with_unstacked_warp(data,
  ...
  warp_shape = torch.tensor(warp_x.shape).to(settings['device'])
  ...
  warp_batch = torch.arange(warp_shape[0], dtype=torch.int32).reshape(warp_shape[0:1],*torch.ones_like(warp_shape[1:]).tolist()).to(settings['device'])

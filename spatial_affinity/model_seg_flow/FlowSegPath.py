    ...
    def forward(self, feature_pyramid1, feature_pyramid2, training=True):
        ...
        for level, (features1, features2) in reversed(
            ...
            if self._init_flow and flow_up is None:
                ...
                if self._num_context_up_channels:#32
                    num_channels = int(self._num_context_up_channels)#32
                    context_up = torch.zeros([batch_size, num_channels, height, width]).to(self._device)
...
#        #flows.insert(0, net_utils.upsample(flows[0], is_flow=True))
    ...
    def _build_upsample_layers(self, num_channels):
        ...
        layers = []
        for unused_level in range(self._num_levels):#6
            layers.append(
                nn.ConvTranspose2d(
    ...
    def _build_flow_layers(self):
        ...
        for i in range(0, self._num_levels):#6
            ...
            if i <= self._num_levels - self._seg_num_levels + 1:
                if self._dual_seg:
    ...
    def _build_segment_model(self):
        ...
        block_layers = [128, 128, 96, 64, self._num_context_up_channels]#32
        
        for i in range(0, self._seg_num_levels):#4
            layers = nn.ModuleList()

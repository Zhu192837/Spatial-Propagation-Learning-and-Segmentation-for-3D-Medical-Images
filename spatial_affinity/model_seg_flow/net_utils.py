...
def loss_feature_consistency(f1, warped_f2, flows, compute_level):
    loss = []
    for i in compute_level:
        loss.append(compute_l1_loss(f1[i],
                                    warped_f2[i], flows[i]))
...
def loss_ssim_weight(img1, img2, flows, compute_level, device):
    ...
    if ssim_weight is not None and ssim_weight > 0:
        for i in compute_level:
            ...
            B, _, H, W = img1_.shape
            ssim_error, avg_weight = weighted_ssim(warped_img2, 
                                                   img1_, 
...
def loss_segmentation(mask1, mask2, segs_f, compute_level, device):
    loss = []
    ...
    for i in compute_level:
        ...
        loss_b = []
        for b in range(B):
            if torch.sum(mask1[b]).item() >= 0:
                mask1_ = upsample(mask1[b:(b+1)], is_flow=False, scale_factor=factor, align_corners=False)
                loss_b.append(criterion(segs_f[0][i][b:(b+1)], mask1_))
            else:
                loss_b.append(torch.tensor(0.0).to(device))
            if len(segs_f[1]) != 0 and torch.sum(mask2[b]).item() >= 0:
                mask2_ = upsample(mask2[b:(b+1)], is_flow=False, scale_factor=factor, align_corners=False)
                loss_b.append(criterion(segs_f[1][i][b:(b+1)], mask2_))
            else:
                loss_b.append(torch.tensor(0.0).to(device))
        loss.append(torch.stack(loss_b).mean())
    
    return torch.stack(loss).sum()

def loss_seg_consistency(mask1, mask2, flows_f, flows_b, compute_level, device):
    loss = []
    criterion = nn.BCEWithLogitsLoss()
    B, C, H, W = mask1.shape
    for i in compute_level:
        ...
        for b in range(B):
            if torch.sum(mask1[b]).item() >= 0:
                mask1_ = upsample(mask1[b:(b+1)], is_flow=False, scale_factor=factor, align_corners=False)
                warped_m1 = resample(mask1_, flow_to_warp(flows_b[i][b:(b+1)]))
                recy_m1 = resample(warped_m1, flow_to_warp(flows_f[i][b:(b+1)]))
                loss_b.append(criterion(recy_m1, mask1_))
            else:
                loss_b.append(torch.tensor(0.0).to(device))
            if torch.sum(mask2[b]).item() >= 0:
                mask2_ = upsample(mask2[b:(b+1)], is_flow=False, scale_factor=factor, align_corners=False)
                warped_m2 = resample(mask2_, flow_to_warp(flows_f[i][b:(b+1)]))
                recy_m2 = resample(warped_m2, flow_to_warp(flows_b[i][b:(b+1)]))
                loss_b.append(criterion(recy_m2, mask2_))
            else:
                loss_b.append(torch.tensor(0.0).to(device))
        loss.append(torch.stack(loss_b).mean())
    return torch.stack(loss).sum()
